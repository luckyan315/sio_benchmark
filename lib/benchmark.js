...
var Benchmark = exports = module.exports = function(opts){
  ...
  // test ping pong
  this.nRecvPong = 0;
  this.nIntervalTime = opts.t || 20000;
  this.max = 0;
  this.avg = 0;
...
(function(){
  ...
  this.run = function(){
    ...
    this.send('run', { nMsgs: nMsgPerWorker, nClients: this.nClientsPerWorker, uri: this.dest, intervalTime: this.nIntervalTime });
  };
  ...
  // subscribe event
  this.subEvent = function(event, cb){
    if(cb && typeof cb === 'function') {
      throw TypeError('Not support callback yet!');
    }
    var self = this;
    this.send('subEvent', { event: event });
  };

  this.pubEvent = function(event, data){
    this.send('pubEvent', { event: event, message: data });
  };
  ...
  this.onanswer = function(worker, msg){
    this.nRecvPong++;
    var data = msg.data;
    var delta = (new Date()).getTime() - data.start;
    ...
    // debug('******[delta] %d', delta);
    debug('[conn] %d, [nAns] %d [delta] %d, [deltaMax] %d [deltaMin] %d, [avg] %d', this.nConnected, this.nRecvPong, delta, this.max, this.min, Math.round(this.avg));
  };
  ...
  // @api private
  this.onsubevent = function(worker, msg){
