...
(function(){
  this.run = function(data){
    ...
    for(var i = 0; i < Math.ceil(this.nRequests/nClients); ++i){
      this.send('stress tests');
    }

    function checkComplete(){
      if(this.nSendMsg < this.nRequests) {
        debug('worker ' + process.pid + ' already send ' + this.nSendMsg + ' messages...');
        return setTimeout(checkComplete, 200);        
      }

      debug('worker ' + process.pid + ' totally send ' + this.nSendMsg + ' messages.');
      this.emit('complete')      
    }
